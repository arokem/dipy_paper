...
In the previous sections we saw how the reconstruction models have a uniform API and can be called in similar ways. For example, they all have an odf() method. This design gives us the opportunity to create utility functions where the model is one of the parameters. \texttt{peaks\_from\_model} is a multipurpose function which can be used to a) find the maxima (peaks) of the ODFs, b) find the directions of the maxima in the ODFs, c) compress the ODFs as spherical harmonics and d) calculate many metrics simultaneously and on the fly, e.g. generalized fractional anisotropy, without the need of creating all ODFs at once.
\begin{python}
from dipy.reconst.odf import peaks_from_model
pmd = peaks_from_model(model=odf_model,
                       data=data,
                       sphere=sphere,
