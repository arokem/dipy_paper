...
\end{equation}
\noindent where $\Delta s$ is the propagation step size which should be at least smaller than the voxel size and $\mathbf{v}$ is the propagation direction. Furthermore, EuDX uses trilinear interpolation for the calculation the next direction integrating directional information from the surrounding voxels.
...
eu = EuDX(a=FA, ind=peak_indices, seeds=start_rois,
          sphere=sphere, a_low=0.2)
tensor_streamlines = [line for line in eu]
\end{python}
The input parameter \texttt{seeds} can be given as an integer and this will generate random seeds in the entire volume or it can be given as $N\times 3$ array of seed points. The instance of EuDX returns back an iterator.
