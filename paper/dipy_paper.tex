...
in Python) eco-system of software.
...
TODO: Need a diagram with the base (numpy, nibabel) and the rest of the modules on top.

Dipy contains the following modules

\begin{figure}
\includegraphics[scale=0.45]{Figures/module_structure2.eps}
\centering{}
\caption{Diagram showing the major Dipy modules and dependencies.\label{Fig:module_structure}}
\end{figure}
...
In this sections we describe some of the tools available for processing 
...
voxel more than once, we only record counts and elements for the first point in
...
fiber tracking procedure. This can happen very easily using the \texttt{lenght}
...
All figures created in this paper are generated by our own visualization tools 
which can be used for most parts of the diffusion analysis pipeline. We have 
developed a minimal, lightweight and up to the point module which is based 
on the Visualization Toolkit (VTK)[REF] and we call \texttt{fvtk}. The main 
idea is that window can have one or more renderers. A renderer can have none, 
one or more actors. Examples of actors are a sphere, line, point or a complete 
set of streamlines. You basically add actors in a renderer and in that way you 
can visualize the forementioned objects e.g. sphere, line etc. The windows can
...
fvtk.show(renderer) 
\end{python}
The most commonly used visualization functions are given in \ref{fvtk_table}.

\begin{table}[th] \processtable{List of visualization functions\label{fvtk_table}} 
{\begin{tabular}{rr}
Name & Usage \\ \hline
ren & create renderer\\ \hline
add & add actor to renderer\\ \hline
rm  & remove actor from renderer \\ \hline
rm\_all & remove all actors from renderer \\ \hline
show & create window and show renderer \\ \hline
record & save frame or frames \\ \hline
line & actor of one or more streamlines \\ \hline
pretty\_line & same as line but with streamtubes \\ \hline
point & actor points as small spheres \\ \hline
tensor & actor of tensor ellipsoids \\ \hline
sphere\_funcs & actor for ODF visualization \\ \hline
