...
human brain \emph{in vivo} non-invasively. Many methods
...
programming interface. We have implemented classical signal reconstruction techniques, such as
the diffusion tensor model and deterministic fiber tractography. In addition,
cutting edge novel reconstruction techniques are implemented, such as constrained
...
, informative visualizations, as well as file-handling routines
...
in Python is relatively straight-forward. Taken together, these properties of the language
...
source (BSD license), simple and well documented. The recent
...
and the availability of other complementing neuroimaging modules\footnote{\url{http://nipy.org}}
make Dipy an ideally positioned addition to the existing scientific tool stack.
...
cases when both standard Python and Numpy/Scipy are not performant enough for
the task at hand. Cython converts Python code into C extensions by
interpreting added static type declarations. The last non-optional dependency for Dipy is
Nibabel\footnote{\url{http://nipy.org/nibabel}} which is used for loading and
saving medical imaging file formats.

Other libraries, not used for analysis but required for
visualization of diffusion data and of the results of
analyses, include Matplotlib\footnote{\url{http://matplotlib.org}} (for 2D and
3D plotting) and Python-VTK\footnote{\url{http://vtk.org}} (for
...
IPython\footnote{\url{http://ipython.org}} as an interactive Python shell for
...
tractography algorithms and visualization of these streamlines.
...
Dipy is a true international project where contributions from anywhere in the
world are welcomed.  In order to ensure the highest quality while still
allowing the code-base to grow, contributors are required to follow
certain policies. For example, if a scientist makes a new discovery --- even
one of substantial importance --- and wants to share the code, it will only be
integrated in the main project after extended testing as well as peer code
review from at least two members of the team.

Through this discipline and work ethic, Dipy has attracted an international
and multi-departmental team of contributors from different levels of education
(Master students, PhD students, Post-Docs and Professors) spanning the fields
of Computer Science, Medicine, Applied Mathematics, Biomedical Engineering and
Psychology.
...
the results of the analysis. In subsequent sections, we use this
terminology to explain the analysis code and interpretation of data.
...
are encoded in so-called \emph{"b-vectors"}. These are unit vectors that describe the
...
applied. Different algorithms are used to determine the placement of these b-vectors
 (e.g. \citep{jones-etal:99, Caruyer2013}).

 While we are ultimately interested in the identification of the trajectories
 of bundles of axons, which are the long fiber-like part of a nerve cell along
 which electrical impulses are conducted from cell to cell (and whose size is
 on the $\mu$m scale), the measurement is conducted on a much larger
 ...
 of \emph{"fascicles"} of nerves, which are relatively large (mm-cm scale)
 bundles of axons travelling through the white matter together. One of the
 major achievements of this field is that it is now possible to reliably and
 accurately identify major fascicles in individual experimental participants or
 patients \citep{mori-atlas}. These major fascicles are also known as
 \emph{"tracts"}. This is a term taken from neuroanatomy and describes a group
 of neuronal axons within the central nervous system (mm scale).
...
muscle fibers, prostate fibers and even celery fibers \citep{Numano2006}. "Fiber bundles" denote
...
underlying fibers. Streamlines are also sometimes referred to as "tracks". These
...
reproducible research \citep{Donoho2010}.
...
management system to perform revision control. Changes are made via a Pull
...
tests to exercise all corner case \citep{Maximilien2003}.
...
provides support for ANALYZE (plain, SPM99, SPM2), GIFTI,
...
\texttt{voxel\_size} is (2, 2, 2), corresponding to a volume of $8~mm^3$.
...
representation and create a DEC (Directionally Encoded Color) map \citep{pierpaoli-jezzard-etal:96, pajevic-pierpaoli:99} which is also known as color FA (see Fig.~\ref{Fig:dti_metrics}):
\begin{python}
...
non-physical and are precluded. The other is that CSD assumes that only a few of the fODF values will be large. Applying these two constraints allows fitting the SH basis up to very high orders, in essence fitting more parameters than data allow.
...
\subsubsection{EuDX.}
Euler Delta Crossings \citep{Garyfallidis_thesis} is the first tracking method that was implemented in Dipy. We created an algorithm that has many
...
\begin{figure*}[!htb]
\includegraphics[scale=0.46]{Figures/qb.eps}
\centering{}
\caption{Left: An example of EuDX streamline tracking applied on a real
  human brain dataset and color-coded with a standard orientation colormap. Middle: showing the QuickBundles centroids with random colors. Right: showing the clusters color-coded with their corresponding centroid color. \label{Fig:pretty_streamlines}}
\end{figure*}
...
first method allows the user to provide the distribution evaluated on a discrete set of possible tracking directions. For example the fODF (fiber orientation distribution)
...
specific cluster. An example of QuickBundles, applied on a human brain dataset described in \citep{Fortin2012}, using the same parameters as shown
in the code listing above, is shown in Fig.~\ref{Fig:pretty_streamlines}.
...
dramatically after the 0.6 which took place on April 2013 (see Fig.~\ref{Fig:gh_stats}),
...
\caption{Participation in the Dipy github repository. The cumulative number of
  unique contributors has been extracted from the git log command and tallied. Many
...
is our hope that many will choose to share their code and join the developers team. We are confident that
...
We are greatful to Amandine Pelletier and Gwenaelle Catheline from Universit\'{e} Bordeaux 2 who provided the fornix data of Fig.~\ref{Fig:fornix}.
