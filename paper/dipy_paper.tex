...
% Did you ask Ian if he could pay?
% Otherwise, it would be better for me and us to be last author. For
% grants and justification of our work, it would be great.
%
\history{}
...
non-invasively measure the diffusion signal in different directions
and locations in the brain. Many methods
...
Fiber tracking, Deterministic tractography, Probabilistic
...
other body tissue with fiber-like structure. DMRI acquires one or more
$T_{2}$ reference images, and a collection
...
\emph{Mission}: The purpose of Dipy is to make it easier to do better diffusion MR
imaging research. We aim to build software that is clearly written, clearly
explained and well tested. These are a good fit for the underlying
ideas and a natural home for collaboration.
...
are encoded in so-called \emph{"b-vectors"}. These are unit {\bf DO
  THEY REALLY NEED TO BE UNIT VECTORS? DSI AND MULTISHELL?} vectors
that describe the direction relative to the scanner coordinate frame
in which the gradients are applied. Different algorithms are used to
determine the placement of these b-vectors (e.g. \citep{jones-etal:99}).
...
generates so-called \emph{streamlines}, which imaginary lines that approximate the
...
\centering{}
\caption{Dipy is one of the main projects of the Nipy community and
  depends strongly
  on Numpy, Scipy and Cython. This diagram further shows the major Dipy
  sub-modules.\label{Fig:module_structure}}
\end{figure}
...
\subsection{Load/Save data in Dipy}\label{loadsave}
...
\subsection{Background removal in Dipy}
...
the foreground and 0s in the background. {\bf WE NEED A FIGURE TO
  ILLUSTRATE THIS}

\subsection{Gradient Table in Dipy}\label{gtab}
...
the dMRI experiment. The gradient unit {\bf AGAIN SURE ABOUT UNIT
  VECTORS?} directions are often referred as
...
\texttt{gtab.b0s\_mask}. {\bf I suggest adding a small example of 6
  directions bvec/bval example to illustrate this. Limiting case for
  DTI. You could also, while at it do a small visualization on the
  sphere like we did at the CreateSummerSchool}
...
relation {\bf I don't think Stejskal Tanner showed this. STanner came
  up with the pulse gradient spin echo sequence. Callaghan came up
  with the Fourier relationship between S and P. Are you sure that you
do not need the $2\pi$ in your equation?}
\begin{equation}
S(\mathbf{q},t)=S_{0}\int
P(\mathbf{r},t)e^{i\mathbf{q}\cdot\mathbf{r}}d\mathbf{r}\label{eq:fourier}
\end{equation}
...
displacement wave vector. $\mathbf{q}$ is parallel to the applied magnetic
gradient $\mathbf{g}$, which in turn are directly related with the b-vectors
discussed in section \ref{gtab}.  {\bf You must say the link between q
  and g... g is the unit vector, q is the 3D displacement in q-space.}
In theory, if we apply the Fourier
...
\begin{equation}
P(\mathbf{r},t)=\frac{1}{\sqrt{4\pi
    t^{3}\mid\mathbf{D}\mid}}exp(-\frac{\mathbf{r}^{T}\mathbf{D}^{-1}\mathbf{r}}{4t})
\end{equation}
\noindent where $\mathbf{D}$ is known as the diffusion tensor. This tensor is a
$3x3$ positive definite symmetric matrix (6 unknowns) {\bf Do we have
  room to put the matrix form of tensor D and coefficients dxx, dyy,
  etc?}  that can be
completely described by a centered ellipsoid with 3 principal axes and
...
impactful variability in brain structure. {\bf If space needed, I
  believe we can significantly reduce this section. Although quite a
  nice read, it can be shorthen without affecting content and the message.}
...
{\bf suggestion: shouldn't you have a small snippet of code or a word
  on fetching data, even for people that do not have their own?}
...
\end{python}
{\bf If room in the paper, I would put a simple 1x3 figure with MD,
  FA, color-FA}
...
fitting \citep{chung-lu-etal:06} as well as Riemanian modeling-based
...
\subsection{Diffusion Spectrum Imaging in Dipy}\label{dsi}
...
complex signal $S$ with its module $|S|$ in Eq.~\ref{eq:fourier} and apply the
Fourier transform:
\begin{eqnarray}
P(\mathbf{r}) & = &
S_{0}^{-1}\int|S(\mathbf{q})|\exp(-i2\pi\mathbf{q}\cdot\mathbf{r})d\mathbf{q}\label{eq:P_modulus}
\end{eqnarray}
{\bf This equation is inconsistent with Eq. 1 because of the
  $2\pi$. Please uniform}
In DSI this triple-integral is approximated in a discrete way and the
PDF P comes out for every single voxel as a 3D array. We can obtain
$P$ using:
\begin{python}
...
\end{python}
With this method we do not need to store all the PDFs at once. If it
is absolutely necessary to save all the PDFs then we advise to create
a Numpy memory-map that will store the are as binary file on
disk. Memory-mapped files are used
for accessing small segments of large files on disk, without reading
the entire file into memory.

Recently, an alternative method for DSI was proposed by
\citep{canales-rodriguez-etal:10} using a deconvolution technique
...
\end{equation}
\noindent This defines the diffusion orientation density function
(ODF) for DSI which
...
on the lattice. {\bf Suggestion: put the corresponding ODF in Fig. 2
  as well as the maxima. This can be a 1x4 figure or 2x2 figure}
Nonetheless, Dipy for reasons explained before does not store
...
\centering{}
\caption{Volumetric rendering of the diffusion propagator of a
  $60\,^{\circ}$ crossing with standard DSI(left) and DSI with
  deconvolution(right). {\bf Can you put the ground truth 60 deg
    directions in the figure? It would help} \label{Fig:pdf}}
\end{figure}


\subsection{Q-ball Imaging in Dipy}
...
there have been several cases of using spherical harmonics to describe
...
\psi_{QBI}(\theta, \phi) = \sum_{j=1}^{R} 2\pi \frac{c_j}{S_0}
P_{l(j)} (0) Y_{j} (\theta, \phi)
\end{equation}
{\bf Is this equation really necessary?}
...
Eq. \label{eq:ODF_DSI}, without the $r^2$ term. Hence, these ODFs are
not properly normalized by the solid angle term. Hence, Dipy proposes
Constant Solid Angle analytical solution more recently proposed by
...
and $16$ SH series have $R = 15, 28, 45$, and $153$ coefficients
respectively.
{\bf A word of the supported SH bases? No concensus in the
  literature. We have implemented basis of Descoteaux (also part of
  the FiberNav) and MRtrix.}

In summary, Dipy implements 3 different Q-ball methods in
module \texttt{dipy.reconst.shm}: a)
\citep{descoteaux-angelino-etal:07}, b) \citep{aganj-lenglet-etal:10}
and c) \citep{tristan-vega-westin-etal:09}. For
...
\end{python}
Important points here are that SH order is set to $6$ and the regularization
parameter $0.006$, as suggested and optimized in
\citep{descoteaux-angelino-etal:06c}. Furthermore, because the
...
\subsection{Constrained Spherical Deconvolution in Dipy}

QBI-based techniques just described reconstruct the \emph{diffusion
...
SD was first introduced by \citep{tournier-calamante-etal:04}. With
...
harmonic order l: $\mathbf{s}_l=\mathbf{R}_l\cdot\mathbf{f}_l$.
{\bf Not sure that these mathematical details really add something. I
  find it more confusing than anything.}
The spherical
...
%This is known as \emph{super-resolution}.
This CSD method can be accessed in Dipy using:
\begin{python}
...
% \begin{equation}
% \psi_{QBI}(\mathbf{u})=\displaystyle\int_{|w|=1} R(\mathbf{u} \cdot
% \mathbf{w}) \psi_{SDT}(\mathbf{w}) dw\label{eq:Conv}
% \end{equation}
...
% spherical harmonics.
% \begin{equation}
% \psi_{SDT}(\mathbf{u})=\displaystyle\sum_{j=1}^{R}2\pi P_{l_{j}}(0)
% \frac{c_j}{f_j}Y_{j}(\mathbf{u})\label{eq:ODF_SDT}
% \end{equation}
The idea here is that an ODF, for example the analytical Q-ball ODF
can be formed by the convolution between the single fiber
diffusion ODF kernel  and the true fiber ODF.
Therefore, the deconvolution can recover a sharper ODF and as before,
using spherical harmonics the derivation is straightforward. Further
explanation of the formula is found in
\cite{descoteaux:08,descoteaux-deriche-etal:09}. In Dipy,
\begin{python}
from dipy.reconst.csdeconv import
...
smallest eigen-value over the largest eigen-value. Both spherical deconvolution
...
\texttt{CsaOdfModel} and \texttt{CsdModel} in of a region in the
centrum semiovale showing crossings between the corpus callosum,
corticospinal tract and the superior longitudinal fasciculus.


\subsection{Extracting Peaks from Dipy models}
...
{\bf to be consistent with our terminology. I believe we should use
  Streamline  Tracking here.}
\subsection{Deterministic tracking in Dipy}
...
points $s$. {\bf $s$ used here and $\delta s$ later. Is this wanted? This
  can be confusing.}
...
\end{python}
{\bf Why give a tensor example here? I would argue to suggest to our
  readers and users that the tensor model should ONLY be used to
  compute metrics and NOT tract (unless they have 20 or less
  measurements). I would put a CSD model here. You can keep FA as
  underlying map but again, I would not put 0.2 as this will create
  holes.... maybe 0.1 or at least mention something...}
...
\subsection{Probabilistic tracking in Dipy}
{\bf To be done?... Maybe we skip it for now. Work under development?}

\section{Post-tracking}\label{post_tracking}
{\bf Don't like Post-tracking. Suggestion: Fiber analysis and
  post-processing}
In the following sections we describe some of the tools available for processing
streamlines after these have been created by the tracking module.

\subsection{Streamline clustering in Dipy}\label{quickbundles}
...
or even to count the number streamlines that go through another ROI,
as recently done in brain \emph{connectomics}
\citep{hagmann-leila-etal:08}. For this
...
calculate track densities \citep{Calamante20101233} or used for
...
{\bf Extend... Re-iterate the mission quickly. How can someone get
  involved? }
...
% This is information for the coauthors of this paper. This section
% will be removed from the last version of this paper.
...
%   \textrm{MDF}(s, t) & = & \min(d_{\textrm{direct}}(s, t),
%   d_{\textrm{flipped}}(s, t))\label{eq:direct_flip_distance}.
% \end{eqnarray}
