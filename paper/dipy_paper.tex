...
\emph{Diffusion MRI} (dMRI) \citep{stejskal-tanner:65,
  merboldt-hanicke-etal:85, lebihan-breton:85, taylor-bushell:85, callaghan:91}
is an MRI technique \citep{} that provides information about the structure of
neuronal pathways found in the white matter and other body tissue with
fiber-like structure. DMRI acquires one or more $T_{2}$ reference images, and a
collection of diffusion-weighted images, in which $T_{2}$ signal is attenuated
according to the diffusivity of water along prescribed gradient directions
\citep{behrens-johansen-berg:09, jones:10}. Because diffusion is hindered
across nerve fiber membranes and less along the length of nerve fibers, the
...
inferred from the measurement. This has led to many applications of the method,
...
\footnote{\url{/http://nose.readthedocs.org/en/latest/}} is used for unit
testing and sphinx \footnote{\url{http://sphinx-doc.org/}} for automatic
...
the dMRI analysis pipeline from the local voxel reconstruction of orientation
distribution functions to streamline generation and visualization.
...
of the MR instrument, but we can change the other three parameters and
...
white matter tissue. Using the assumption that diffusion is a Gaussian process,
and relying on the antipodal symmetry of the measurement (that is, the
measurement cannot differentiate diffusion in any direction from diffusion in
the exact opposite direction), the dODF can be modeled with 6 parameters,
describing the variance and covariance of the Gaussian diffusion along the
three primary axes. From these parameters, several useful measures can be
derived: the primary diffusion direction is the primary eigen-value of the
tensor, $\lambda_{1}$ and it is the direction in which variance in the
...
can be predicted basedcon these statistics, suggesting that they are reporting
on meaningful and impactful variability in brain structure.
...
univariate measures, such as the fractional anisotropy (FA) \cite{Basser1996}:
\begin{python}
...
\end{python}
In DSI this triple-integral in \ref{eq:P_modulus} is approximated in a discrete
way and the PDF P is discretized in every voxel as a 3D array.  We can obtain
$P$ using:
\begin{python}
...
which will store the PDF in each voxel as binary file on disk. Memory-mapped
...
we will need a set of spherical coordinate. Here is how we can obtain this ODF:
\begin{python}
...
function.  We assume that $R$ is derived from a prolate tensor [NEED TO EXPLAIN
'PROLATE'!]. The eigenvalues of this tensor are estimated from the voxels with
FA $> 0.7$. The input parameter \texttt{response} is a tuple with two
parameters: a) the eigen-values of the tensor and b) the estimated average S0
signal for those voxels. The response function is usually estimated from the
corpus callosum area. For further information on how to initialize a CSD
analysis, please read our examples in \texttt{dipy.org}.
...
(GFA)\cite{cohen-adad-etal:08}, without the need of creating all ODFs at
...
gfa = pmd.gfa
\end{python}
The \texttt{model} parameter can be any of the models discussed in the previous
sections e.g. \texttt{dsi\_model}. The \texttt{relative\_peak\_threshold}
parameter enforces that only peaks greater than
\texttt{relative\_peak\_threshold}$*m$ will be returned, where $m$ is the value
of the largest peak. \texttt{min\_separation\_angle} sets the threshold for the
minimum angular distance in degrees between two peaks. If the peaks are closer
than this threshold only the larger of the two is returned. This two parameters
help to get robust fiber directions when the ODFs are
...
One of the tantalizing prospects of DWI is the promise that combining
information about the local microstructure across different voxels may provide
information about large-scale organization of the brain. In particular,
researchers have been trying to use various algorithms to track along the
...
the structure and function of the connectome is a central goal of modern
neuroscience. This premise has led to major investment in data-collection
projects aimed at characterizing the connectome, based on DWI, as well as
functional MRI (fMRI) measurements \cite{VanEssen2013}.

Tracking algorithms, used to infer these connections divide into to major
...
The other class of algorithms is probabilistic, in that it considers the local
...
Depending on the initial number of seeds and other tracking parameters fiber
tracking algorithms can generate a great number of densely packed streamlines
(often more than one million). These results are often difficult to interpert
and the large amount of data is often difficult to interact with. As a solution
to this problem Dipy implements a recent and efficient clustering algorithm for
streamlines. This is called QuickBundles (QB) \cite{Garyfallidis2012} and it
can be used to simplify large datasets in a couple of minutes.  When using
QuickBundles we need first to instantiate the object with 3 parameters. The
first parameter is the set of initial streamlines to be clustered, the second
is the distance threshold that defines the cluster size and the third is the
level of detail for each streamline. For example, in the the code snippet below
we used \texttt{pts=18} which means that before QB starts the clustering
procedure the streamlines will be downsampled so that each one has the same
number of points (here 18) and equal segments. This preprocessing step is a
prerequisite for QB.
\begin{python}
...
Fig.~\ref{Fig:pdf},~\ref{Fig:ten_csa_csd},~\ref{Fig:pretty_streamlines} are
generated by our the visualization tools that are part of Dipy. These can be
used for most parts of the diffusion analysis pipeline. We have developed a
minimal, lightweight module, based on the Visualization Toolkit (VTK)[REF],
which we call \texttt{fvtk}. The main principles of the modules operation is
that a window can have one or more renderers.  A renderer can have none, one or
more actors. Examples of actors are a sphere, line, point or a complete set of
streamlines. When a specific actor is added to a renderer it can be
visualized. Windows can be created by either the \texttt{show} function which
creates a visible window or the \texttt{record} function which creates a
temporary window only for the purpose of using it to render the objects and
save the frames disk. The renderer holds all the actors, i.e. the visible
objects. Here is a simple example to visualize some streamlines with different
colors:
\begin{python}
...
fvtk.show(renderer)
\end{python}
A quick reference to some of the visualization functions are given in
Tab.~\ref{fvtk_table}.
...
development of these software tools through a mailing list (\url{XXX}), and
this is also a good place to discuss other Python-/neuroimaging-related issues
(e.g.: XXX [ADD the permutation testing example]).
...
contributors to the code-base and the number of contributors is growing from
release to release. 

One major source of support for this community comes in the form of the
...
Eleftherios Garyfallidis is funded by XXX.  
